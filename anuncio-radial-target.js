(() => {
  ...
  function mount() {
    ...
    if (!card) return;
    card.dataset.radialTargetReady = "1";

    const title = card.querySelector(".play-top h3");
    const text = card.querySelector(".play-top .muted-main");
    if (title) title.textContent = "5. Detective del anuncio";
    if (text) text.textContent = "Analiza cada caso y descubre c\u00F3mo persuade el anuncio radial.";
  ...
  function renameActivityCard() {
    app.querySelectorAll(`[data-oa="${TARGET_GAME}"]`).forEach((button) => {
      ...
      if (heading) heading.textContent = "5. Detective del anuncio";
      if (copy) copy.textContent = "Resuelve casos y descubre producto, p\u00FAblico, intenci\u00F3n y recursos persuasivos.";
    });
  }
